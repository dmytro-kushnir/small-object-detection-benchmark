...
**Temporal majority (optional inference prior):** [`track_yolo_video.py`](../scripts/inference/track_yolo_video.py) and [`track_rfdetr_video.py`](../scripts/inference/track_rfdetr_video.py) support **`--temporal-state-window K`** applied *after* soft state-priority. Each track’s label is a sliding majority over the last `K` frame-level classes (ties → current frame). Analytics include `temporal_state_smooth`. This is **not** part of Idea 1 COCO val/test; use it for **OOD / qualitative** exploration. It can rebalance `states` when predictions **flicker**; it **cannot** invent trophallaxis if the detector rarely predicts class 1. Sweep helper: [`scripts/run_camponotus_ood_temporal_sweep.sh`](../scripts/run_camponotus_ood_temporal_sweep.sh); copy-paste patterns: [`cli_commands.md`](cli_commands.md).
