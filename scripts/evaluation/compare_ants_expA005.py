#!/usr/bin/env python3
"""EXP-A005: RF-DETR ants metrics vs YOLO 768 baseline (single compare block)."""
...
EVAL_NOTE = (
    "RF-DETR full-image predict vs Ultralytics YOLO vanilla predict@imgsz=768. "
    "Input resolution and backbone differ; interpret mAP and throughput accordingly."
)
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="YOLO 768 metrics JSON",
    )
    p.add_argument(
        "--compare",
        type=str,
        default=str(root / "experiments/results/ants_expA005_rfdetr_metrics.json"),
        help="RF-DETR metrics JSON",
    )
    p.add_argument(
        "--out",
        type=str,
        default=str(root / "experiments/results/ants_expA005_rfdetr_vs_yolo.json"),
    )
    ...
    payload: dict[str, Any] = {
        ...
        "paths": {
            "ants_expA002b_imgsz768_metrics": path_for_artifact(pb, root),
            "ants_expA005_rfdetr_metrics": path_for_artifact(pc, root),
        },
    ...
    d = pair["deltas"]
    print()
    print("=== EXP-A005 RF-DETR vs YOLO768 (RF-DETR − YOLO) ===")
